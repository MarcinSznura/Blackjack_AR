...
        #region PRIVATE_METHODS
        ...
        {
            if (FindObjectOfType<GameMaster>().GetCurrentStage() == 4
                || FindObjectOfType<GameMaster>().GetCurrentStage() == 3)
            {
                return true;
            }
            else
...
            foreach (Collider component in colliderComponents)
            ...
            if (!countedInThisRound && CanIRead())
            {
                countedInThisRound = true;
                FindObjectOfType<GameMaster>().IncreasePlayerScore(cardValue);
                FindObjectOfType<GameMaster>().playerHand.Add(cardValue);
            }
